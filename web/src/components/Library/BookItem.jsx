                ...
                <p className="text-sm mb-3 font-normal text-gray-700 dark:text-gray-400 truncate">
                    {props.author && props.onAuthorClick ? (
                        <span
                            role="button"
                            tabIndex={0}
                            onClick={() => props.onAuthorClick(props.author)}
                            onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); props.onAuthorClick(props.author); } }}
                            className="cursor-pointer hover:underline focus:outline-none"
                        >
                            {t("book.by_author", {author: props.author})}
                        </span>
                    ) : (
                        t("book.by_author", {author: props.author || t("book.unknown_author")})
                    )}
                </p>
                ...
                {props.showProgress &&
                    <div className="text-xs mt-auto">
                        ...
                        <div className='flex flex-row items-center justify-between w-full'>
                            ...
                            <div>
                                <ActionsBookLibraryButton id={props.internalID} onSuccess={props.onReadingStatusChanged} allowNoteEditing={props.allowNoteEditing} totalPages={props.totalPages} author={props.author} title={props.title} subtitle={props.subtitle} isbn={props.isbn} description={props.description} readingStatus={props.readingStatus}/>
                            </div>
                        </div>
                    </div>
                }
